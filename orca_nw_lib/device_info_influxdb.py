...
organization = "ramya-org"
bucket = "my-first-bucket"
token = "Li7pjXQKsCgLEgDI_edJzWN5RlR3lGXN4q5wIVZCGy5QCWrnXOHea5ON8Qtr2WPtsLoqVQuFPnlnKTT7msYafQ=="
...
def get_device_info(device_ip=host1["ip"]):
    ...
    try:
        ...
        logger.info("Device details retrieved: %s", details)
        
        influx_data = {
            "hostname": metadata["sonic-device-metadata:DEVICE_METADATA"]["DEVICE_METADATA_LIST"][0]["hostname"],
            "img_name": details["img_name"],
            "mgt_intf": details["mgt_intf"],
            "mgt_ip": details["mgt_ip"],
            "hwsku": details["hwsku"],
            "mac": details["mac"],
...
def send_to_influxdb(device_name: str, details: dict):
    ...
    # client = InfluxDBClient(url=urls, token=token, org=organization)
    
    # try:
    #     # Create a write API for synchronous writes
    #     write_api = client.write_api(write_options=SYNCHRONOUS)
        
    #     # Create the InfluxDB point with device tags and fields
    #     point = Point("device_metrics").tag("device", device_name)
        
    #     # Add fields from the details dictionary
    #     for key, value in details.items():
    #         point.field(key, value)
        
    #     # Set the time for the point (current UTC time with nanosecond precision)
    #     point.time(datetime.datetime.now(datetime.timezone.utc), WritePrecision.NS)
        
    #     # Write the point to the InfluxDB bucket
    #     write_api.write(bucket=bucket, org=organization, record=point)
        
    #     print(f"Metrics for {device_name} sent to InfluxDB.")
